...
def RVGAIA(run=False, load=False, **kwargs):
	...
	Gdata = GAIAdata(os.path.join(here, 'Gaia_astrometry_BH3.gaia'),skip=2,units='mas')
	Vdata = RVData(os.path.join(here, 'Gaia_RVs_BH3.rdb'),skip=2,units='kms')
...
if __name__ == '__main__':
	model = RVGAIA(run=False, steps=80000)
	res = kima.load_results(model,diagnostic=True)
